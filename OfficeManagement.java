...
class Office implements Serializable
{
	...
	public Office(int id, String name, float salary, long contact_no, String email_id)
	{
		this.id = id;
		this.name = name;
		this.salary = salary;
...
{	
	static void display(ArrayList<Office> al)
	{
		System.out.println("\n            --------------Employee List---------------\n");
		System.out.println(String.format("%-10s%-15s%-10s%-20s%-10s", "ID","Name","salary","contact-no","Email-Id"));
		for(Office o : al)
		{
			System.out.println(String.format("%-5s%-20s%-10s%-15s%-10s",o.id,o.name,o.salary,o.contact_no,o.email_id));
		}
	}
	...
	public static void main(String[] args)
	{
		...
		try{
			...
			f = new File("EmployeeList.txt");
			if(f.exists())
			{
				fis = new FileInputStream(f);
				ois = new ObjectInputStream(fis);
				al = (ArrayList<Office>)ois.readObject();
			}
		...
		do
		{
			...
			case 1:System.out.println("\nEnter the following details to ADD list:\n");
				...
				email_id = sc.next();
				al.add(new Office(id, name, salary, contact_no, email_id));
				display(al);
				break;
